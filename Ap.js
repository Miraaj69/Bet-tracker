...
export default function App() {
  ...
  async function loadHTML() {
    try {
      ...
      // Try localUri first, then uri
      const sourceUri = asset.localUri || asset.uri;
      console.log('Asset URI:', sourceUri);

      if (!sourceUri) {
        console.error('No URI found for asset');
        return;
      }

      // Copy to documentDirectory to ensure readable path
      const dest = FileSystem.documentDirectory + 'bt_app.html';
