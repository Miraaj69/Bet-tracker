...
function HeroCard({tPL, roi, wr, won, lost, pending, t}) {
  ...
  useEffect(() => {
    animVal.setValue(0);
    Animated.timing(animVal, {toValue:1, duration:1100, useNativeDriver:false}).start();
    const id = animVal.addListener(({value}) => setDisplayNum(Math.round(value * tPL)));
    return () => animVal.removeListener(id);
  }, [tPL]);

  // Glow pulse when profit
  const pulseAnim = useRef(new Animated.Value(1)).current;
  useEffect(() => {
  ...
  return (
    <Animated.View style={{
      ...
      backgroundColor: plC + '11',
      borderWidth:1, borderColor: plC + '35',
      shadowColor: plC, shadowOpacity: isProfit ? 0.38 : 0.18,
      shadowRadius:30, shadowOffset:{width:0, height:8},
      transform:[{scale: pulseAnim}],
    }}>
      <View style={{flexDirection:'row', justifyContent:'space-between', alignItems:'flex-start', marginBottom:4}}>
        <Text style={{fontSize:11, color:'rgba(255,255,255,0.42)', fontWeight:'600', letterSpacing:1.2, textTransform:'uppercase'}}>Total P / L</Text>
        <View style={{backgroundColor: plC+'22', borderRadius:8, paddingHorizontal:9, paddingVertical:3}}>
          <Text style={{fontSize:10, color:plC, fontWeight:'700'}}>ROI {pct(roi)}</Text>
        </View>
      </View>
      <Text style={{fontSize:46, fontWeight:'900', color:plC, letterSpacing:-2, marginBottom:16, includeFontPadding:false}}>
        {sign}{absStr}
      </Text>
      <View style={{flexDirection:'row', borderTopWidth:1, borderTopColor:'rgba(255,255,255,0.08)', paddingTop:14}}>
        {[
          ...
          <View key={i} style={{flex:1, alignItems:'center'}}>
            ...
            <Text style={{fontSize:9, color:'rgba(255,255,255,0.35)', marginTop:2, fontWeight:'500'}}>{item.label}</Text>
          </View>
        ))}
      </View>
    </Animated.View>
...
function QuickActions({onAddBet, onQuickAdd, onStats, onExport, t}) {
  const actions = [
    {icon:'➕', label:'Add Bet',   color:'#30d158', bg:'rgba(48,209,88,0.12)',  border:'rgba(48,209,88,0.25)',  fn:onAddBet},
    {icon:'⚡', label:'Quick Add', color:'#0a84ff', bg:'rgba(10,132,255,0.12)', border:'rgba(10,132,255,0.25)', fn:onQuickAdd},
    {icon:'📊', label:'Stats',     color:'#bf5af2', bg:'rgba(191,90,242,0.12)', border:'rgba(191,90,242,0.25)', fn:onStats},
    {icon:'📤', label:'Export',    color:'#ff9f0a', bg:'rgba(255,159,10,0.12)', border:'rgba(255,159,10,0.25)', fn:onExport},
  ];
  return (
    <View style={{marginBottom:16}}>
      <Text style={{fontSize:11, color:'rgba(255,255,255,0.32)', fontWeight:'600', letterSpacing:1, textTransform:'uppercase', marginBottom:10}}>Quick Actions</Text>
      <View style={{flexDirection:'row', gap:10}}>
        {actions.map((a,i) => (
          <TouchableOpacity key={i} onPress={a.fn} activeOpacity={0.72}
            style={{flex:1, backgroundColor:a.bg, borderRadius:18, borderWidth:1, borderColor:a.border, paddingVertical:14, alignItems:'center', gap:6}}>
            <Text style={{fontSize:22}}>{a.icon}</Text>
            <Text style={{fontSize:10, color:a.color, fontWeight:'700', letterSpacing:0.2}}>{a.label}</Text>
          </TouchableOpacity>
        ))}
      </View>
    </View>
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false}>
      ...
      <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginBottom:18,marginTop:6}}>
        <View>
          <Text style={{fontSize:13,color:'rgba(255,255,255,0.38)',fontWeight:'500'}}>{greeting} 👋</Text>
          <Text style={{fontSize:20,fontWeight:'900',color:t.text,letterSpacing:-0.5}}>BetTracker Pro</Text>
        </View>
        <View style={{alignItems:'center',gap:3}}>
          <View style={{width:42,height:42,borderRadius:21,backgroundColor:'rgba(10,132,255,0.18)',borderWidth:1.5,borderColor:'rgba(10,132,255,0.38)',justifyContent:'center',alignItems:'center'}}>
            <Text style={{fontSize:20}}>👤</Text>
          </View>
          <View style={{backgroundColor:'rgba(48,209,88,0.18)',borderRadius:5,paddingHorizontal:6,paddingVertical:1}}>
            <Text style={{fontSize:8,color:'#30d158',fontWeight:'700'}}>{bets.length} bets</Text>
          </View>
        </View>
      </View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{marginBottom:16}} contentContainerStyle={{gap:8,flexDirection:'row'}}>
        {[
          ...
          <View key={i} style={{backgroundColor:s.bg,borderRadius:20,borderWidth:1,borderColor:s.border,paddingHorizontal:16,paddingVertical:12,alignItems:'center',minWidth:80}}>
            <Text style={{fontSize:16,marginBottom:4}}>{s.icon}</Text>
            <Text style={{fontSize:15,fontWeight:'800',color:s.color,letterSpacing:-0.3}}>{s.value}</Text>
            <Text style={{fontSize:9,color:'rgba(255,255,255,0.38)',marginTop:3,fontWeight:'600',letterSpacing:0.5,textTransform:'uppercase'}}>{s.label}</Text>
          </View>
        ))}
      </ScrollView>
      ...
      {/* ── Recent Bets ── */}
      {bets.length>0&&<Card t={t} style={{marginBottom:12}}>
        <SectionHead title="⚡ Recent Bets" sub="Last 3" t={t}
          right={
            <TouchableOpacity onPress={onGoToBets} style={{backgroundColor:'rgba(10,132,255,0.15)',borderRadius:8,paddingHorizontal:10,paddingVertical:4}}>
              <Text style={{fontSize:10,color:'#0a84ff',fontWeight:'700'}}>See All →</Text>
            </TouchableOpacity>
          }
        />
  ...
  return (
    <SafeAreaProvider>
      ...
      <View style={{flex:1,backgroundColor:t.bg}}>
        ...
        {showFab&&(
          <TouchableOpacity
            style={{position:'absolute',top:0,left:0,right:0,bottom:0,zIndex:200,backgroundColor:'rgba(0,0,0,0.48)'}}
            activeOpacity={1}
        ...
        {/* ── FAB action items ──────────────────────────────────────────── */}
        {showFab&&(
          <View style={{position:'absolute',bottom:118,left:0,right:0,zIndex:300,alignItems:'center',gap:14}}>
            {[
              ...
              {icon:'🎫',label:'Bet Slip',   color:'#ffd60a', fn:()=>{setShowSlip(true);setShowFab(false);}},
              {icon:'📋',label:'Templates',  color:'#bf5af2', fn:()=>{setShowTemplates(true);setShowFab(false);}},
            ].map((item,i)=>(
              <TouchableOpacity key={i} onPress={item.fn} activeOpacity={0.85}
                style={{flexDirection:'row',alignItems:'center',gap:14}}>
                <View style={{backgroundColor:'rgba(24,24,28,0.96)',borderRadius:14,borderWidth:1,borderColor:'rgba(255,255,255,0.13)',paddingHorizontal:18,paddingVertical:10,shadowColor:'#000',shadowOpacity:0.5,shadowRadius:14}}>
                  <Text style={{color:'#fff',fontSize:14,fontWeight:'700'}}>{item.label}</Text>
                </View>
                <View style={{width:52,height:52,borderRadius:16,backgroundColor:item.color,justifyContent:'center',alignItems:'center',shadowColor:item.color,shadowOpacity:0.6,shadowRadius:16,shadowOffset:{width:0,height:4}}}>
