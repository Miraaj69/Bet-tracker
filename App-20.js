...
function QuickActions({onAddBet, onQuickAdd, onStats, onExport, t}) {
  ...
  const actions = [
    {icon:'➕', label:'Add Bet',   color:'#30d158', glow:'rgba(48,209,88,0.35)',  iconBg:'rgba(48,209,88,0.18)',  fn:onAddBet},
    {icon:'⚡', label:'Quick Add', color:'#0a84ff', glow:'rgba(10,132,255,0.35)', iconBg:'rgba(10,132,255,0.18)', fn:onQuickAdd},
    {icon:'📊', label:'Stats',     color:'#bf5af2', glow:'rgba(191,90,242,0.35)', iconBg:'rgba(191,90,242,0.18)', fn:onStats},
    {icon:'📤', label:'Export',    color:'#ff9f0a', glow:'rgba(255,159,10,0.35)', iconBg:'rgba(255,159,10,0.18)', fn:onExport},
  ];
  const pressIn  = i => Animated.spring(scales[i], {toValue:0.91, useNativeDriver:true, speed:50, bounciness:4}).start();
  const pressOut = i => Animated.spring(scales[i], {toValue:1,    useNativeDriver:true, speed:28, bounciness:8}).start();
  return (
    <View style={{marginBottom:20}}>
      <Text style={{fontSize:11, color:'rgba(255,255,255,0.28)', fontWeight:'700', letterSpacing:1.4, textTransform:'uppercase', marginBottom:12}}>Quick Actions</Text>
      <View style={{flexDirection:'row', gap:10}}>
        {actions.map((a,i) => (
          <Animated.View key={i} style={{flex:1, transform:[{scale:scales[i]}]}}>
            <TouchableOpacity
              ...
              style={{
                borderRadius:22,
                overflow:'hidden',
                backgroundColor:'rgba(28,28,32,0.82)',
                borderWidth:1,
                borderColor:'rgba(255,255,255,0.10)',
                paddingVertical:18,
                alignItems:'center',
                gap:9,
                shadowColor: a.glow.replace('0.35','1'),
                shadowOpacity:0.22,
                shadowRadius:12,
                shadowOffset:{width:0,height:4},
              }}
            >
              ...
              <Text style={{fontSize:10, color: a.color, fontWeight:'700', letterSpacing:0.3, textAlign:'center'}}>{a.label}</Text>
              {/* Subtle glow line at top */}
              <View style={{position:'absolute',top:0,left:'20%',right:'20%',height:1,backgroundColor:a.color,opacity:0.35,borderRadius:1}}/>
            </TouchableOpacity>
          </Animated.View>
        ))}
      </View>
  ...
  return (
    <SafeAreaProvider>
      ...
      <View style={{flex:1,backgroundColor:t.bg}}>
        ...
        {/* ── FAB action items ──────────────────────────────────────────── */}
        {showFab&&(
          <View style={{position:'absolute',bottom:118,left:0,right:0,zIndex:300,alignItems:'flex-end',paddingRight:20,gap:16}}>
            {[
              {icon:'➕',label:'Add Bet',    color:'#30d158', iconBg:'rgba(48,209,88,0.22)',   fn:()=>{setEditBet(null);setShowModal(true);setShowFab(false);}},
              {icon:'⚡',label:'Quick Add',  color:'#0a84ff', iconBg:'rgba(10,132,255,0.22)',  fn:()=>{setShowQuick(true);setShowFab(false);}},
              {icon:'🎫',label:'Bet Slip',   color:'#ffd60a', iconBg:'rgba(255,214,10,0.22)',  fn:()=>{setShowSlip(true);setShowFab(false);}},
              {icon:'📋',label:'Templates',  color:'#bf5af2', iconBg:'rgba(191,90,242,0.22)',  fn:()=>{setShowTemplates(true);setShowFab(false);}},
            ].map((item,i)=>(
              <TouchableOpacity key={i} onPress={item.fn} activeOpacity={0.82}
                style={{flexDirection:'row',alignItems:'center',gap:14}}>
                {/* Label pill — frosted glass */}
                <View style={{
                  ...
                  <Text style={{color:'#fff',fontSize:14,fontWeight:'700',letterSpacing:0.2}}>{item.label}</Text>
                </View>
                {/* Icon button — premium frosted */}
                <View style={{
                  width:56, height:56,
                  borderRadius:18,
                  ...
                  {/* Top glow line */}
                  <View style={{position:'absolute',top:0,left:'15%',right:'15%',height:1.5,backgroundColor:item.color,opacity:0.5,borderRadius:1}}/>
