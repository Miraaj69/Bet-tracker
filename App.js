...
function SplashScreen({ onFinish }) {
  ...
  useEffect(() => {
    const sp = (v, to, d=0, t=80, f=9) => Animated.spring(v, { toValue:to, tension:t, friction:f, delay:d, useNativeDriver:true });
    const ea = (v, to, dur=420, d=0, e=Easing.out(Easing.cubic)) => Animated.timing(v, { toValue:to, duration:dur, delay:d, easing:e, useNativeDriver:true });
  ...
  return (
    <Animated.View style={[styles.splash, { opacity: exitOpacity, transform: [{ translateY: slideY }] }]}>
      ...
      <Animated.Text style={[styles.splashTitle, { opacity:titleOpacity, transform:[{translateY:titleY}] }]}>BetTracker Pro</Animated.Text>
      <Animated.Text style={[styles.splashTag, { opacity:tagOpacity, transform:[{translateY:tagY}] }]}>Smart · Fast · Disciplined</Animated.Text>
...
export default function App() {
  ...
  if (Platform.OS === 'web') {
    return <SafeAreaView style={styles.container}><Text style={{color:'#fff'}}>BetTracker Pro</Text></SafeAreaView>;
  }
