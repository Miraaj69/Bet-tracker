...
import {
  ...
  StatusBar, FlatList, Share, Animated, Vibration
} from 'react-native';
  ...
  return (
    <SafeAreaProvider>
      ...
      <View style={{flex:1,backgroundColor:t.bg}}>
        ...
        {/* Undo delete bar */}
        {undo&&(
          <View style={{position:'absolute',bottom:90,left:16,right:16,zIndex:9000,backgroundColor:'rgba(30,30,40,0.97)',borderRadius:16,borderWidth:1,borderColor:'rgba(255,255,255,0.12)',padding:12,flexDirection:'row',alignItems:'center',gap:10,shadowColor:'#000',shadowOpacity:0.5,shadowRadius:20}}>
        ...
        <SafeAreaView edges={['bottom']} style={{backgroundColor:t.nav}}>
          <View style={{flexDirection:'row',alignItems:'center',paddingHorizontal:6,paddingVertical:6,borderTopWidth:1,borderColor:t.cardB,gap:5}}>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{flex:1}}>
              <View style={{flexDirection:'row',gap:1}}>
                {TABS.map(([icon,name],i)=>(
                  <TouchableOpacity key={i} onPress={()=>setTab(i)} style={{backgroundColor:tab===i?'rgba(10,132,255,0.22)':'transparent',borderRadius:12,paddingVertical:5,paddingHorizontal:7,alignItems:'center',borderWidth:1,borderColor:tab===i?'rgba(10,132,255,0.32)':'transparent',minWidth:40}}>
                    <Text style={{fontSize:12}}>{icon}</Text>
                    <Text style={{fontSize:tab===i?7:6.5,color:tab===i?'#0a84ff':t.muted,fontWeight:tab===i?'700':'500'}}>{name}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </ScrollView>
            <View style={{gap:4}}>
              <TouchableOpacity onPress={()=>setShowFab(p=>!p)} style={{width:44,height:44,borderRadius:13,backgroundColor:showFab?'rgba(255,69,58,0.9)':'#0a84ff',justifyContent:'center',alignItems:'center',shadowColor:'#0a84ff',shadowOpacity:0.5,shadowRadius:10}}>
                <Text style={{color:'#fff',fontSize:20}}>{showFab?'✕':'＋'}</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={()=>setShowDrawer(true)} style={{width:44,height:26,borderRadius:9,backgroundColor:t.nav,borderWidth:1,borderColor:t.cardB,justifyContent:'center',alignItems:'center'}}>
