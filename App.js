...
function NavTabItem({icon, name, active, onPress, t, accent}) {
  ...
  const handlePress = () => {
    Animated.sequence([
      Animated.timing(scale,{toValue:0.80,duration:70,useNativeDriver:true}),
      Animated.spring(scale,{toValue:1,tension:320,friction:10,useNativeDriver:true}),
    ]).start();
    onPress();
  };
  return (
    <TouchableOpacity onPress={handlePress} activeOpacity={1} style={{alignItems:'center',flex:1,paddingVertical:4,minWidth:44}}>
      <Animated.View style={{alignItems:'center',transform:[{scale}]}}>
        <Text style={{fontSize:22,lineHeight:28}}>{icon}</Text>
        <Text style={{fontSize:10,color:active?accent:t.muted,fontWeight:active?'700':'500',letterSpacing:-0.1,marginTop:1}}>{name}</Text>
        {active&&<View style={{width:4,height:4,borderRadius:2,backgroundColor:accent,marginTop:2}}/>}
      </Animated.View>
    </TouchableOpacity>
  );
}
  ...
  return (
    <SafeAreaProvider>
      ...
      <View style={{flex:1,backgroundColor:t.bg}}>
        ...
        {/* Bottom nav — iOS style with centered FAB */}
        <SafeAreaView edges={['bottom']} style={{position:'absolute',bottom:0,left:0,right:0,zIndex:100,backgroundColor:t.nav}}>
          <View style={{borderTopWidth:0.5,borderColor:t.cardB,paddingTop:8,paddingBottom:4,paddingHorizontal:4}}>
            <View style={{flexDirection:'row',alignItems:'flex-end',justifyContent:'space-around'}}>
              ...
              {/* Center FAB */}
              <View style={{width:64,alignItems:'center',justifyContent:'flex-end',paddingBottom:4}}>
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    shadowOpacity:0.6,shadowRadius:18,shadowOffset:{width:0,height:4},
                    borderWidth:2.5,borderColor:'rgba(255,255,255,0.20)',
                    marginBottom:4,
                  }}
                >
                  <Text style={{color:'#fff',fontSize:30,fontWeight:'200',lineHeight:34,includeFontPadding:false}}>
                    {showFab?'✕':'＋'}
                  </Text>
                </TouchableOpacity>
              </View>
              ...
              <TouchableOpacity onPress={()=>setShowDrawer(true)} activeOpacity={0.7}
                style={{alignItems:'center',flex:1,paddingVertical:4,minWidth:44}}>
                <Text style={{fontSize:22,lineHeight:28}}>☰</Text>
                <Text style={{fontSize:10,color:t.muted,fontWeight:'500',marginTop:1}}>More</Text>
              </TouchableOpacity>
