...
// ─── THEMES ──────────────────────────────────────────────────────────────────
const THEMES = {
  amoled: { bg:'#000', bg2:'#080808', card:'rgba(255,255,255,0.06)', cardB:'rgba(255,255,255,0.09)', text:'#fff', sub:'rgba(255,255,255,0.52)', muted:'rgba(255,255,255,0.28)', inp:'rgba(255,255,255,0.07)', inpB:'rgba(255,255,255,0.14)', nav:'rgba(0,0,0,0.97)', accent:'#0a84ff', tint:'#4f8cff', tint2:'#7c4dff', grad1:'rgba(10,60,140,0.18)', grad2:'rgba(90,40,180,0.12)' },
  dark:   { bg:'#0c0c1e', bg2:'#1a1a2e', card:'rgba(255,255,255,0.07)', cardB:'rgba(255,255,255,0.11)', text:'#fff', sub:'rgba(255,255,255,0.52)', muted:'rgba(255,255,255,0.3)', inp:'rgba(255,255,255,0.09)', inpB:'rgba(255,255,255,0.14)', nav:'rgba(8,8,20,0.97)', accent:'#5e5ce6', tint:'#7c78ff', tint2:'#26c2ff', grad1:'rgba(80,60,220,0.16)', grad2:'rgba(20,140,220,0.10)' },
  light:  { bg:'#eef2ff', bg2:'#fff', card:'rgba(255,255,255,0.82)', cardB:'rgba(0,0,0,0.07)', text:'#08081a', sub:'rgba(0,0,0,0.52)', muted:'rgba(0,0,0,0.33)', inp:'rgba(0,0,0,0.045)', inpB:'rgba(0,0,0,0.1)', nav:'rgba(232,238,255,0.97)', accent:'#0a84ff', tint:'#5aa8ff', tint2:'#7c5cff', grad1:'rgba(100,140,255,0.12)', grad2:'rgba(160,100,255,0.08)' },
  forest: { bg:'#0a1a0f', bg2:'#0f2318', card:'rgba(48,209,88,0.07)', cardB:'rgba(48,209,88,0.14)', text:'#e8ffe8', sub:'rgba(200,255,200,0.52)', muted:'rgba(150,210,150,0.35)', inp:'rgba(48,209,88,0.07)', inpB:'rgba(48,209,88,0.18)', nav:'rgba(8,22,12,0.97)', accent:'#30d158', tint:'#34d399', tint2:'#22c55e', grad1:'rgba(48,209,88,0.12)', grad2:'rgba(52,211,153,0.08)' },
};
...
function GradientBackground({t}) {
  ...
  useEffect(()=>{
    const loop1 = Animated.loop(Animated.sequence([
      Animated.timing(orb1,{toValue:1,duration:4000,useNativeDriver:false}),
      Animated.timing(orb1,{toValue:0,duration:4000,useNativeDriver:false}),
    ]));
    const loop2 = Animated.loop(Animated.sequence([
      Animated.timing(orb2,{toValue:1,duration:5500,useNativeDriver:false}),
      Animated.timing(orb2,{toValue:0,duration:5500,useNativeDriver:false}),
    ]));
    loop1.start(); loop2.start();
    return ()=>{loop1.stop();loop2.stop();};
  },[]);
  const op1 = orb1.interpolate({inputRange:[0,1],outputRange:[0.7,1]});
  const op2 = orb2.interpolate({inputRange:[0,1],outputRange:[0.5,0.9]});
  return (
    <View style={{position:'absolute',top:0,left:0,right:0,bottom:0,pointerEvents:'none',overflow:'hidden'}}>
      <Animated.View style={{position:'absolute',top:-120,left:-80,width:320,height:320,borderRadius:160,backgroundColor:t.grad1,opacity:op1}}/>
      <Animated.View style={{position:'absolute',top:SH*0.3,right:-100,width:280,height:280,borderRadius:140,backgroundColor:t.grad2,opacity:op2}}/>
      <View style={{position:'absolute',bottom:-60,left:'20%',width:240,height:240,borderRadius:120,backgroundColor:t.grad1,opacity:0.5}}/>
    </View>
  );
}

// ─── SKELETON LOADER (Change #7) ─────────────────────────────────────────────
function SkeletonPulse({width, height, borderRadius=10, style}) {
  const anim = useRef(new Animated.Value(0.3)).current;
  useEffect(()=>{
    const loop = Animated.loop(Animated.sequence([
      Animated.timing(anim,{toValue:1,duration:850,useNativeDriver:false}),
      Animated.timing(anim,{toValue:0.3,duration:850,useNativeDriver:false}),
    ]));
    loop.start(); return ()=>loop.stop();
  },[]);
  return (
    <Animated.View style={[{width,height,borderRadius,backgroundColor:'rgba(255,255,255,0.09)',opacity:anim},style]}/>
  );
}

function BetCardSkeleton({t}) {
  return (
    <View style={{backgroundColor:t.card,borderRadius:20,borderWidth:1,borderColor:t.cardB,padding:14,marginBottom:10,borderLeftWidth:3.5,borderLeftColor:'rgba(255,255,255,0.08)'}}>
      <View style={{flexDirection:'row',gap:10,marginBottom:12}}>
        <SkeletonPulse width={38} height={38} borderRadius={12}/>
        <View style={{flex:1,gap:7}}>
          <SkeletonPulse width='80%' height={13} borderRadius={6}/>
          <SkeletonPulse width='55%' height={10} borderRadius={6}/>
        </View>
        <View style={{alignItems:'flex-end',gap:6}}>
          <SkeletonPulse width={50} height={22} borderRadius={8}/>
          <SkeletonPulse width={40} height={12} borderRadius={5}/>
        </View>
      </View>
      <View style={{flexDirection:'row',gap:6}}>
        <SkeletonPulse width={60} height={18} borderRadius={8}/>
        <SkeletonPulse width={48} height={18} borderRadius={8}/>
      </View>
    </View>
  );
}

function HomeSkeletonLoader({t}) {
  return (
    <View style={{paddingHorizontal:14,paddingTop:16}}>
      <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
        <View style={{gap:7}}><SkeletonPulse width={100} height={11}/><SkeletonPulse width={180} height={26} borderRadius={6}/></View>
        <SkeletonPulse width={80} height={34} borderRadius={17}/>
      </View>
      <SkeletonPulse width='100%' height={160} borderRadius={28} style={{marginBottom:16}}/>
      <SkeletonPulse width='100%' height={120} borderRadius={20} style={{marginBottom:16}}/>
      {[0,1,2].map(i=><BetCardSkeleton key={i} t={t}/>)}
    </View>
  );
}

// ─── IMPROVED GLASSMORPHISM CARD (Change #2) ─────────────────────────────────
...
function GlassLayer({t, borderRadius=22, blurAmount, surface='card', tone}) {
  ...
  // Cards ko clean solid background do — Android par BlurView cards ko washed out bana deta hai
  // BlurView sirf nav aur modal ke liye use karo
  const useBlur = BlurView && (surface === 'nav' || surface === 'modal');

  // Card background: theme ke hisaab se subtle solid color
  const cardBg = light
    ? 'rgba(255,255,255,0.78)'
    : t.bg === THEMES.forest.bg
      ? 'rgba(15,35,20,0.82)'
      : t.bg === THEMES.dark.bg
        ? 'rgba(22,22,48,0.85)'
        : 'rgba(18,18,24,0.88)'; // amoled
  ...
  const overlayStyle = {
    ...
    borderColor: surface === 'modal'
      ...
      : surface === 'nav'
        ? (light ? 'rgba(255,255,255,0.64)' : 'rgba(255,255,255,0.14)')
        : light ? 'rgba(255,255,255,0.55)' : 'rgba(255,255,255,0.08)',
    backgroundColor: useBlur
      ? getGlassOverlay(t, surface)
      : surface === 'card' || surface === 'button'
        ? cardBg
        : t.card,
  };

  // Reflection aur sheen sirf nav/modal par — cards par nahi (warna text dhak jaata hai)
  const reflectionBase = {
    ...StyleSheet.absoluteFillObject,
    borderRadius,
    backgroundColor: light ? 'rgba(255,255,255,0.06)' : 'rgba(255,255,255,0.03)',
  };

  // topGlow — cards par bahut halka rakho
  const topGlowOpacity = surface === 'nav'
    ? (light ? 'rgba(255,255,255,0.18)' : 'rgba(255,255,255,0.08)')
    : surface === 'modal'
      ? (light ? 'rgba(255,255,255,0.22)' : 'rgba(255,255,255,0.10)')
      : (light ? 'rgba(255,255,255,0.10)' : 'rgba(255,255,255,0.04)'); // card — bahut halka
  ...
  const topGlow = {
    ...
    left: 0,
    right: 0,
    height: surface === 'nav' ? '52%' : surface === 'modal' ? '50%' : '38%',
    borderTopLeftRadius: borderRadius,
    borderTopRightRadius: borderRadius,
    backgroundColor: topGlowOpacity,
  };

  // Sheen sirf nav aur modal ke liye — cards par bilkul nahi
  const sheen = surface === 'card' || surface === 'button' ? null : {
    position: 'absolute',
    top: -18,
    ...
    backgroundColor: light ? 'rgba(255,255,255,0.14)' : 'rgba(255,255,255,0.05)',
  ...
  const edgeHighlight = {
    ...
    height: 1,
    borderRadius,
    backgroundColor: light ? 'rgba(255,255,255,0.48)' : 'rgba(255,255,255,0.12)',
  };
  ...
  const bottomShade = {
    ...
    right: 0,
    bottom: 0,
    height: surface === 'modal' ? '40%' : '32%',
    borderBottomLeftRadius: borderRadius,
    borderBottomRightRadius: borderRadius,
    backgroundColor: light ? 'rgba(0,0,0,0.01)' : 'rgba(0,0,0,0.12)',
  };

  const gradId = `glass-grad-${surface}-${String(tint).replace(/[^a-zA-Z0-9]/g, '')}-${borderRadius}`;

  // Tint gradient — cards par bahut subtle rakho
  const tintAlphaA = surface === 'card'
    ? (light ? 0.10 : 0.12)
    : (light ? 0.22 : 0.20);
  const tintAlphaB = surface === 'card'
    ? (light ? 0.03 : 0.06)
    : (light ? 0.07 : 0.10);
  const tintAlphaEnd = surface === 'card'
    ? (light ? 0.04 : 0.08)
    : (light ? 0.08 : 0.14);
    ...
    <Svg pointerEvents="none" style={StyleSheet.absoluteFillObject} width="100%" height="100%">
      <Defs>
        <LinearGradient id={gradId} x1="0" y1="0" x2="1" y2="1">
          <Stop offset="0%" stopColor={withAlpha(tint, tintAlphaA)} />
          <Stop offset="45%" stopColor={withAlpha(tint, tintAlphaB)} />
          <Stop offset="75%" stopColor="rgba(255,255,255,0)" />
          <Stop offset="100%" stopColor={withAlpha(t.tint2 || tint, tintAlphaEnd)} />
        </LinearGradient>
      </Defs>
  ...
  if (!useBlur) {
    return (
      <>
        ...
        {sheen && <View pointerEvents="none" style={sheen}/>}
...
// ─── UI PRIMITIVES ────────────────────────────────────────────────────────────
function Card({t, children, style, onPress, glowColor, accent}) {
  const scale = useRef(new Animated.Value(1)).current;
  const glowAnim = useRef(new Animated.Value(0)).current;
  const pressIn  = () => {
    if(onPress) {
      Animated.spring(scale,{toValue:0.972,useNativeDriver:true,speed:50,bounciness:0}).start();
      Animated.timing(glowAnim,{toValue:1,duration:120,useNativeDriver:false}).start();
    }
  };
  const pressOut = () => {
    if(onPress) {
  ...
  const light = isLightTheme(t);
  const glowColor2 = glowColor || accent || t.accent;
  return (
    <Animated.View style={[
      {
        transform:[{scale}],
        ...
        borderRadius:24,
        borderWidth:1,
        borderColor: light ? 'rgba(255,255,255,0.7)' : 'rgba(255,255,255,0.09)',
        backgroundColor: light ? 'rgba(255,255,255,0.72)' : 'rgba(255,255,255,0.055)',
        ...getDepthStyle(glowColor ? 'floating' : 'card', glowColor2 ? glowColor2 : (light ? 'rgba(15,23,42,0.22)' : '#000')),
      },
      style,
    ]}>
      {/* Top shimmer highlight */}
      <View pointerEvents="none" style={{position:'absolute',top:0,left:16,right:16,height:1,backgroundColor: light ? 'rgba(255,255,255,0.9)' : 'rgba(255,255,255,0.18)',borderRadius:1,zIndex:3}}/>
      {/* Side glow on press */}
      {onPress && <Animated.View pointerEvents="none" style={{position:'absolute',inset:0,borderRadius:24,borderWidth:1.5,borderColor:glowColor2,opacity:glowAnim,zIndex:2}}/>}
      <View style={{borderRadius:24,overflow:'hidden'}}>
        <GlassLayer t={t} surface='card' borderRadius={24} blurAmount={28} tone={glowColor2}/>
        <TouchableOpacity
...
// ─── EMPTY STATE ILLUSTRATED (Change #12) ────────────────────────────────────
function EmptyStateSVG({t}) {
  ...
  return (
    <Animated.View style={{transform:[{translateY:float}],alignItems:'center',marginBottom:8}}>
      <Svg width="160" height="140" viewBox="0 0 160 140" fill="none">
        <Circle cx="80" cy="75" r="55" fill={t.accent} opacity="0.08"/>
        <Circle cx="80" cy="75" r="40" fill={t.accent} opacity="0.06"/>
        <Rect x="45" y="35" width="70" height="85" rx="10" fill={t.accent} opacity="0.15"/>
        <Rect x="45" y="35" width="70" height="85" rx="10" stroke={t.accent} strokeWidth="1.5" fill="none" opacity="0.5"/>
        <Rect x="62" y="29" width="36" height="14" rx="7" fill={t.accent} opacity="0.7"/>
        <Rect x="57" y="60" width="46" height="5" rx="2.5" fill={t.accent} opacity="0.3"/>
        <Rect x="57" y="72" width="36" height="5" rx="2.5" fill={t.accent} opacity="0.2"/>
        <Rect x="57" y="84" width="42" height="5" rx="2.5" fill={t.accent} opacity="0.25"/>
        <Circle cx="116" cy="40" r="14" fill="#30d158" opacity="0.9"/>
        <Path d="M110 40H122M116 34V46" stroke="#fff" strokeWidth="2.5" strokeLinecap="round"/>
        <Circle cx="32" cy="55" r="3" fill="#ffd60a" opacity="0.6"/>
        <Circle cx="138" cy="90" r="2.5" fill="#bf5af2" opacity="0.5"/>
        <Circle cx="28" cy="100" r="2" fill={t.accent} opacity="0.4"/>
      </Svg>
    </Animated.View>
  );
}

// ─── QUICK STATS ROW (Change #10) ─────────────────────────────────────────────
function QuickStatsRow({bets, t}) {
  if(!bets.length) return null;
  const won = bets.filter(b=>b.result==='won').length;
  const total = bets.filter(b=>b.result!=='pending').length;
  const wr = total>0?(won/total*100):0;
  const totalStake = bets.reduce((s,b)=>s+(b.stake||0),0);
  const roi = totalStake>0?bets.reduce((s,b)=>s+(b.profit||0),0)/totalStake*100:0;
  const bestOdds = bets.reduce((m,b)=>Math.max(m,parseFloat(b.odds)||0),0);
  const streak = (()=>{
    let c=0,tp='';
    for(const b of [...bets].sort((a,x)=>new Date(x.date)-new Date(a.date))){
      if(b.result==='pending') continue;
      if(!tp){tp=b.result;c=1;} else if(b.result===tp){c++;} else break;
    }
    return {c,tp};
  })();

  const items = [
    {label:'Win Rate', value:wr.toFixed(0)+'%', icon:'🎯', color:'#30d158'},
    {label:'ROI', value:(roi>=0?'+':'')+roi.toFixed(1)+'%', icon:'📈', color:roi>=0?'#30d158':'#ff453a'},
    {label:'Best Odds', value:bestOdds.toFixed(2), icon:'⚡', color:'#ffd60a'},
    {label:'Streak', value:streak.c+'×'+(streak.tp==='won'?'W':streak.tp==='lost'?'L':''), icon:'🔥', color:streak.tp==='won'?'#30d158':'#ff453a'},
  ];
    ...
    <View style={{flexDirection:'row',gap:7,marginBottom:16}}>
      {items.map((item,i)=>(
        <View key={i} style={{
          ...
          <View pointerEvents="none" style={{position:'absolute',top:0,left:8,right:8,height:1,backgroundColor:'rgba(255,255,255,0.18)',borderRadius:1}}/>
          <Text style={{fontSize:14,marginBottom:3}}>{item.icon}</Text>
          <Text style={{fontSize:13,fontWeight:'900',color:item.color,letterSpacing:-0.3}}>{item.value}</Text>
          <Text style={{fontSize:7.5,color:t.muted,fontWeight:'800',letterSpacing:0.5,textTransform:'uppercase',marginTop:2}}>{item.label}</Text>
        </View>
      ))}
    </View>
  );
      ...
      <Animated.View style={{
        ...
        <View style={{
          ...
          <View style={{flexDirection:'row', justifyContent:'space-between', alignItems:'flex-start', marginBottom:10}}>
            <View>
              <Text style={{fontSize:9, color:'rgba(255,255,255,0.3)', fontWeight:'800', letterSpacing:2.5, textTransform:'uppercase'}}>Total P&L</Text>
              <Text style={{fontSize:10, color:'rgba(255,255,255,0.4)', marginTop:5, fontWeight:'600',letterSpacing:0.1}}>{won}W · {lost}L · {pending} pending</Text>
            </View>
            <View style={{backgroundColor: plC+'24', borderRadius:12, paddingHorizontal:12, paddingVertical:6, borderWidth:1, borderColor:plC+'40'}}>
              <Text style={{fontSize:13, color:plC, fontWeight:'900', letterSpacing:-0.2}}>{(roi>=0?'+':'') + pct(roi,1)} ROI</Text>
            </View>
          </View>

          <Text style={{
            fontSize:68, fontWeight:'900', color:plC, letterSpacing:-4,
            marginBottom:10, includeFontPadding:false, lineHeight:74,
            textShadowColor: isProfit ? 'rgba(48,209,88,0.4)' : 'rgba(255,69,58,0.4)',
            textShadowRadius:22,
          }}>
            {sign}{absStr}
          </Text>
          ...
          <View style={{flexDirection:'row', alignItems:'center', justifyContent:'space-between', marginBottom:16}}>
            <Text style={{fontSize:13, color:streakColor, fontWeight:'900', letterSpacing:0.1}}>{streakText}</Text>
            <View style={{backgroundColor:'rgba(255,255,255,0.07)', borderRadius:10, paddingHorizontal:10, paddingVertical:4, borderWidth:1, borderColor:'rgba(255,255,255,0.09)'}}>
              <Text style={{fontSize:9, color:'rgba(255,255,255,0.55)', fontWeight:'800',letterSpacing:0.5}}>WIN RATE {pct(wr,0)}</Text>
            </View>
          </View>
          ...
          <View style={{flexDirection:'row', gap:8}}>
            {[
              ...
              <View key={i} style={{
                ...
                <Text style={{fontSize:24, fontWeight:'900', color:item.color, letterSpacing:-0.8, textAlign:'center',lineHeight:28}}>{item.value}</Text>
                <Text style={{fontSize:8, color:'rgba(255,255,255,0.28)', fontWeight:'800', letterSpacing:1.2, textTransform:'uppercase', textAlign:'center', marginTop:4}}>{item.label}</Text>
              </View>
            ))}
          </View>
        </View>
...
function Dashboard({bets, t, onAddBet, onQuickAdd, onSettlePending, onGoToBets, onGoToStats}) {
  ...
  if(bets.length===0) return (
    <View style={{alignItems:'center',justifyContent:'center',padding:32,flex:1}}>
      <GradientBackground t={t}/>
      <EmptyStateSVG t={t}/>
      <Text style={{fontSize:24,fontWeight:'900',color:t.text,marginBottom:8,letterSpacing:-0.5}}>No bets yet</Text>
      <Text style={{fontSize:13,color:t.muted,lineHeight:20,textAlign:'center',marginBottom:8}}>Add your first bet to unlock your personal dashboard — P&L, charts, streaks and more.</Text>
      <Text style={{fontSize:11,color:t.sub,marginBottom:28,textAlign:'center'}}>🏏 Cricket · ⚽ Football · 🎾 Tennis · and more</Text>
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false}>
      ...
      <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginBottom:16,marginTop:6}}>
        <View>
          <Text style={{fontSize:11,color:'rgba(255,255,255,0.32)',fontWeight:'700',letterSpacing:0.6}}>{greeting} 👋</Text>
          <Text style={{fontSize:28,fontWeight:'900',color:t.text,letterSpacing:-1.2,lineHeight:32}}>BetTracker Pro</Text>
        </View>
...
function SwipeBetCard({b, onEdit, onDelete, onDuplicate, onMarkResult, t, index=0}) {
  ...
  useEffect(()=>{
    ...
    Animated.parallel([
      Animated.timing(enterAnim,{toValue:1,duration:340,delay,useNativeDriver:true}),
      Animated.spring(enterScale,{toValue:1,tension:200,friction:14,delay,useNativeDriver:true}),
    ]).start();
  },[]);
  ...
  const profitStr = isPending ? '₹'+b.stake : fcs(b.profit||0);
  // Odds indicator: > 2.0 = high value arrow
  const oddsVal = parseFloat(b.odds)||1;
  const oddsColor = oddsVal>=3?'#ff9f0a':oddsVal>=2?'#30d158':'#ffd60a';

  return (
    <Animated.View style={{opacity:enterAnim,transform:[{scale:enterScale}],marginBottom:10}}>
    <View style={{borderRadius:22,overflow:'hidden'}}>
      {/* Swipe bg layers */}
      <View style={{position:'absolute',inset:0,flexDirection:'row',justifyContent:'space-between',alignItems:'center',paddingHorizontal:20,borderRadius:22}}>
        <View style={{backgroundColor:'rgba(10,132,255,0.22)',borderRadius:14,paddingHorizontal:14,paddingVertical:8,flexDirection:'row',alignItems:'center',gap:6}}>
          <Text style={{fontSize:16}}>✏️</Text>
          <Text style={{color:'#0a84ff',fontWeight:'800',fontSize:12}}>Edit</Text>
        </View>
        <View style={{backgroundColor:'rgba(255,69,58,0.22)',borderRadius:14,paddingHorizontal:14,paddingVertical:8,flexDirection:'row',alignItems:'center',gap:6}}>
      ...
      <Animated.View {...panResponder.panHandlers}
        ...
        <View pointerEvents="none" style={{position:'absolute',top:0,left:16,right:16,height:1,backgroundColor:'rgba(255,255,255,0.16)',zIndex:2}}/>
        {/* Left accent bar with glow */}
        <View style={{position:'absolute',left:0,top:0,bottom:0,width:4,backgroundColor:rc,borderTopLeftRadius:22,borderBottomLeftRadius:22,shadowColor:rc,shadowOpacity:0.8,shadowRadius:8,shadowOffset:{width:0,height:0}}}/>
        {/* Subtle sport-color bg tint */}
        <View pointerEvents="none" style={{position:'absolute',top:0,right:0,width:80,bottom:0,backgroundColor:rc,opacity:0.025}}/>

        <View style={{padding:14,paddingLeft:20}}>
          {/* Row 1: Sport icon + Match + Status badge + Profit */}
          <View style={{flexDirection:'row',alignItems:'flex-start',marginBottom:10,gap:10}}>
            <View style={{
              width:42,height:42,borderRadius:14,
              backgroundColor:sport.color+'22',
              justifyContent:'center',alignItems:'center',
              borderWidth:1,borderColor:sport.color+'40',
              shadowColor:sport.color,shadowOpacity:0.3,shadowRadius:8,shadowOffset:{width:0,height:2},
            }}>
              <Text style={{fontSize:22}}>{sport.icon}</Text>
            </View>
            <View style={{flex:1}}>
              <Text style={{fontSize:14,fontWeight:'900',color:t.text,letterSpacing:-0.3,lineHeight:18}} numberOfLines={1}>{b.match}</Text>
              <Text style={{fontSize:11,color:t.sub,marginTop:3,fontWeight:'500'}} numberOfLines={1}>{b.selection}</Text>
            </View>
            <View style={{alignItems:'flex-end',gap:5}}>
              <View style={{backgroundColor:rc+'25',borderRadius:10,paddingHorizontal:9,paddingVertical:3,borderWidth:1,borderColor:rc+'50'}}>
                <Text style={{fontSize:9.5,color:rc,fontWeight:'900',textTransform:'uppercase',letterSpacing:0.5}}>{b.result}</Text>
              </View>
              <Text style={{fontSize:15,fontWeight:'900',color:isPending?t.sub:pc,letterSpacing:-0.4}}>{profitStr}</Text>
            </View>
          </View>

          {/* Row 2: Odds + Market + Tags */}
          <View style={{flexDirection:'row',alignItems:'center',gap:8,marginBottom:9,flexWrap:'wrap'}}>
            {/* Odds badge with value indicator */}
            <View style={{flexDirection:'row',alignItems:'center',gap:3,backgroundColor:oddsColor+'18',borderRadius:8,paddingHorizontal:8,paddingVertical:3,borderWidth:1,borderColor:oddsColor+'35'}}>
              <Text style={{fontSize:10,color:oddsColor,fontWeight:'900'}}>@ {b.odds}</Text>
              <Text style={{fontSize:8,color:oddsColor}}>{oddsVal>=3?'🔥':oddsVal>=2?'↑':'·'}</Text>
            </View>
            {b.market&&<View style={{backgroundColor:t.inp,borderRadius:7,paddingHorizontal:7,paddingVertical:2,borderWidth:1,borderColor:t.inpB}}>
              <Text style={{fontSize:9,color:t.muted,fontWeight:'700'}}>{b.market}</Text>
            </View>}
            {b.betType&&b.betType!=='Single'&&<View style={{backgroundColor:'rgba(191,90,242,0.12)',borderRadius:7,paddingHorizontal:7,paddingVertical:2,borderWidth:1,borderColor:'rgba(191,90,242,0.25)'}}>
              <Text style={{fontSize:9,color:'#bf5af2',fontWeight:'700'}}>{b.betType}</Text>
            </View>}
            {b.livebet&&<View style={{backgroundColor:'rgba(255,69,58,0.18)',borderRadius:6,paddingHorizontal:6,paddingVertical:1,borderWidth:1,borderColor:'rgba(255,69,58,0.35)'}}>
              <Text style={{fontSize:8,color:'#ff453a',fontWeight:'900',letterSpacing:0.5}}>LIVE</Text>
            </View>}
            {b.tags&&b.tags.slice(0,2).map(tag=>(
              <View key={tag} style={{backgroundColor:'rgba(10,132,255,0.10)',borderRadius:7,paddingHorizontal:6,paddingVertical:2}}>
                <Text style={{fontSize:9,color:t.tint||'#4f8cff',fontWeight:'700'}}>#{tag}</Text>
              </View>
            ))}
          </View>
          ...
          <View style={{flexDirection:'row',alignItems:'center',justifyContent:'space-between'}}>
            <View style={{flexDirection:'row',alignItems:'center',gap:10}}>
              <View style={{flexDirection:'row',alignItems:'center',gap:4}}>
                <Text style={{fontSize:9,color:t.muted}}>📅</Text>
                <Text style={{fontSize:9,color:t.muted,fontWeight:'600'}}>{b.date}</Text>
              </View>
            ...
            {isPending&&(
              <View style={{flexDirection:'row',gap:5}}>
                <GlossyPill t={t} label="Won" icon="✓" onPress={()=>{Vibration.vibrate(80);onMarkResult(b.id,'won');}} tone="#30d158" active size='xs'/>
                <GlossyPill t={t} label="Lost" icon="✗" onPress={()=>{Vibration.vibrate([0,40,30,40]);onMarkResult(b.id,'lost');}} tone="#ff453a" active size='xs' destructive/>
              </View>
            )}
            {!isPending&&(
              <View style={{flexDirection:'row',gap:4}}>
          ...
          {b.notes&&<View style={{marginTop:8,backgroundColor:t.inp,borderRadius:9,padding:8,borderWidth:1,borderColor:t.inpB}}>
            <Text style={{fontSize:10,color:t.sub,fontStyle:'italic',lineHeight:15}}>📝 {b.notes}</Text>
          </View>}
        </View>
      </Animated.View>
    </View>
        ...
        ListEmptyComponent={
          <View style={{alignItems:'center',paddingVertical:48,paddingHorizontal:24}}>
            <Svg width="120" height="100" viewBox="0 0 120 100" fill="none">
              <Circle cx="60" cy="52" r="38" fill={t.accent} opacity="0.07"/>
              <Rect x="30" y="28" width="60" height="54" rx="8" stroke={t.muted} strokeWidth="1.5" fill="none" opacity="0.5"/>
              <Path d="M30 40H90" stroke={t.muted} strokeWidth="1.5" opacity="0.4"/>
              <Rect x="40" y="52" width="40" height="4" rx="2" fill={t.muted} opacity="0.25"/>
              <Rect x="40" y="62" width="28" height="4" rx="2" fill={t.muted} opacity="0.15"/>
              <Circle cx="88" cy="28" r="13" fill={t.accent} opacity="0.8"/>
              <Path d="M84 28H92M88 24V32" stroke="#fff" strokeWidth="2" strokeLinecap="round"/>
            </Svg>
            <Text style={{fontSize:16,fontWeight:'900',color:t.text,marginTop:12,letterSpacing:-0.3}}>No bets found</Text>
            <Text style={{fontSize:12,color:t.muted,marginTop:6,textAlign:'center',lineHeight:18}}>Try adjusting your filters{'\n'}or add a new bet to get started</Text>
          </View>
        }
...
// ─── SVG ICONS FOR NAV ────────────────────────────────────────────────────────
// ─── NAV ICON STYLES ─────────────────────────────────────────────────────────
// Style A: Filled (bold, solid shapes)
const NAV_ICONS_FILLED = {
  'Home': ({color}) => (
  ...
  'Insights': ({color}) => (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      ...
      <Rect x="8" y="8" width="3" height="13" rx="1" fill={color}/>
      <Rect x="13" y="11" width="3" height="10" rx="1" fill={color}/>
      <Rect x="18" y="5" width="3" height="16" rx="1" fill={color}/>
    </Svg>
  ),
  'Bets': ({color}) => (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      <Path d="M3 7C3 5.9 3.9 5 5 5H19C20.1 5 21 5.9 21 7V9H3V7Z" fill={color}/>
      <Path d="M3 9H21V19C21 20.1 20.1 21 19 21H5C3.9 21 3 20.1 3 19V9Z" fill={color} opacity="0.8"/>
      <Path d="M7 14H9M11 14H13M15 14H17" stroke="rgba(0,0,0,0.45)" strokeWidth="1.8" strokeLinecap="round"/>
    </Svg>
  ),
  'Profile': ({color}) => (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      <Circle cx="12" cy="8" r="4" fill={color}/>
      <Path d="M4 21C4 17.5 7.58 15 12 15C16.42 15 20 17.5 20 21H4Z" fill={color} opacity="0.8"/>
    </Svg>
  ),
};

// Style B: Outlined (thin stroke, minimal)
const NAV_ICONS_OUTLINED = {
  'Home': ({color}) => (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      <Path d="M4 10.5L12 4L20 10.5V20C20 20.55 19.55 21 19 21H15V15H9V21H5C4.45 21 4 20.55 4 20V10.5Z" stroke={color} strokeWidth="1.7" fill="none" strokeLinejoin="round"/>
    </Svg>
  ),
  'Insights': ({color}) => (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      <Path d="M4 20V14" stroke={color} strokeWidth="2" strokeLinecap="round"/>
      <Path d="M9 20V8"  stroke={color} strokeWidth="2" strokeLinecap="round"/>
      <Path d="M14 20V12" stroke={color} strokeWidth="2" strokeLinecap="round"/>
      <Path d="M19 20V5"  stroke={color} strokeWidth="2" strokeLinecap="round"/>
    </Svg>
  ),
  'Bets': ({color}) => (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      <Rect x="3" y="5" width="18" height="14" rx="2.5" stroke={color} strokeWidth="1.7" fill="none"/>
      <Path d="M3 9H21" stroke={color} strokeWidth="1.7"/>
      <Path d="M7 13H9M11 13H13M15 13H17" stroke={color} strokeWidth="1.7" strokeLinecap="round"/>
    </Svg>
  ),
  'Profile': ({color}) => (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      <Circle cx="12" cy="8" r="4" stroke={color} strokeWidth="1.7" fill="none"/>
      <Path d="M4 20C4 17 7.58 14 12 14C16.42 14 20 17 20 20" stroke={color} strokeWidth="1.7" strokeLinecap="round" fill="none"/>
    </Svg>
  ),
};

// Style C: Rounded Filled (iOS-style rounded square bg icons)
const NAV_ICONS_ROUNDED = {
  'Home': ({color, active, accent}) => (
    <Svg width="26" height="26" viewBox="0 0 26 26" fill="none">
      {active && <Rect x="1" y="1" width="24" height="24" rx="8" fill={accent} opacity="0.25"/>}
      <Path d="M5 11.5L13 5L21 11.5V21C21 21.55 20.55 22 20 22H16V16H10V22H6C5.45 22 5 21.55 5 21V11.5Z" fill={color}/>
    </Svg>
  ),
  'Insights': ({color, active, accent}) => (
    <Svg width="26" height="26" viewBox="0 0 26 26" fill="none">
      {active && <Rect x="1" y="1" width="24" height="24" rx="8" fill={accent} opacity="0.25"/>}
      <Rect x="4" y="15" width="3" height="7" rx="1" fill={color}/>
      <Rect x="9" y="9" width="3" height="13" rx="1" fill={color}/>
      <Rect x="14" y="12" width="3" height="10" rx="1" fill={color}/>
      <Rect x="19" y="6" width="3" height="16" rx="1" fill={color}/>
    </Svg>
  ),
  'Bets': ({color, active, accent}) => (
    <Svg width="26" height="26" viewBox="0 0 26 26" fill="none">
      {active && <Rect x="1" y="1" width="24" height="24" rx="8" fill={accent} opacity="0.25"/>}
      <Rect x="3" y="6" width="20" height="15" rx="3" stroke={color} strokeWidth="1.7" fill="none"/>
      <Path d="M3 10H23" stroke={color} strokeWidth="1.7"/>
      <Path d="M7 14.5H9M11.5 14.5H13.5M16 14.5H18" stroke={color} strokeWidth="1.8" strokeLinecap="round"/>
    </Svg>
  ),
  'Profile': ({color, active, accent}) => (
    <Svg width="26" height="26" viewBox="0 0 26 26" fill="none">
      {active && <Rect x="1" y="1" width="24" height="24" rx="8" fill={accent} opacity="0.25"/>}
      <Circle cx="13" cy="9" r="4" fill={color}/>
      <Path d="M5 21C5 18 8.58 16 13 16C17.42 16 21 18 21 21H5Z" fill={color} opacity="0.85"/>
    </Svg>
  ),
};

// Style D: Duotone (two-tone with opacity layer)
const NAV_ICONS_DUOTONE = {
  'Home': ({color}) => (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      <Path d="M4 10.5L12 4L20 10.5V20C20 20.55 19.55 21 19 21H5C4.45 21 4 20.55 4 20V10.5Z" fill={color} opacity="0.25"/>
      <Path d="M9 21V15H15V21" stroke={color} strokeWidth="1.8" strokeLinejoin="round"/>
      <Path d="M4 10.5L12 4L20 10.5" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
    </Svg>
  ),
  'Insights': ({color}) => (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      <Rect x="3" y="14" width="3" height="7" rx="1" fill={color} opacity="0.3"/>
      <Rect x="8" y="8" width="3" height="13" rx="1" fill={color} opacity="0.45"/>
      <Rect x="13" y="11" width="3" height="10" rx="1" fill={color} opacity="0.6"/>
      <Rect x="18" y="5" width="3" height="16" rx="1" fill={color}/>
    </Svg>
  ),
  'Bets': ({color}) => (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      <Rect x="3" y="5" width="18" height="14" rx="2.5" fill={color} opacity="0.2"/>
      <Rect x="3" y="5" width="18" height="4.5" rx="2.5" fill={color} opacity="0.5"/>
      <Path d="M7 14H9M11 14H13M15 14H17" stroke={color} strokeWidth="1.8" strokeLinecap="round"/>
    </Svg>
  ),
  'Profile': ({color}) => (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      <Path d="M4 21C4 17.5 7.58 15 12 15C16.42 15 20 17.5 20 21H4Z" fill={color} opacity="0.3"/>
      <Circle cx="12" cy="8" r="4" fill={color} opacity="0.9"/>
    </Svg>
  ),
};

// Style E: Sharp / Angular (no rounded corners, geometric)
const NAV_ICONS_SHARP = {
  'Home': ({color}) => (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      <Path d="M3 11L12 3L21 11V21H15V14H9V21H3V11Z" fill={color}/>
    </Svg>
  ),
  'Insights': ({color}) => (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      <Rect x="3" y="13" width="3.5" height="8" fill={color}/>
      <Rect x="8.25" y="7" width="3.5" height="14" fill={color}/>
      <Rect x="13.5" y="10" width="3.5" height="11" fill={color}/>
      <Rect x="18.5" y="4" width="3.5" height="17" fill={color}/>
    </Svg>
  ),
  'Bets': ({color}) => (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      ...
      <Path d="M7 14H9M12 14H14M17 14H19" stroke={color} strokeWidth="2" strokeLinecap="square"/>
    </Svg>
  ),
  'Profile': ({color}) => (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      <Rect x="8" y="3" width="8" height="9" fill={color}/>
      <Path d="M3 21C3 17 7 15 12 15C17 15 21 17 21 21H3Z" fill={color} opacity="0.85"/>
    </Svg>
  ),
};

// Style F: Neon Outline (thin with glow-ready strokes)
const NAV_ICONS_NEON = {
  'Home': ({color}) => (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      <Path d="M4 10.5L12 4L20 10.5V20C20 20.55 19.55 21 19 21H15V15H9V21H5C4.45 21 4 20.55 4 20V10.5Z" stroke={color} strokeWidth="1.5" fill="none" strokeLinejoin="round"/>
      <Path d="M12 4L20 10.5" stroke={color} strokeWidth="1.5" strokeLinecap="round"/>
    </Svg>
  ),
  'Insights': ({color}) => (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      <Path d="M4 20V14" stroke={color} strokeWidth="1.8" strokeLinecap="round"/>
      <Path d="M9 20V8"  stroke={color} strokeWidth="1.8" strokeLinecap="round"/>
      <Path d="M14 20V12" stroke={color} strokeWidth="1.8" strokeLinecap="round"/>
      <Path d="M19 20V5"  stroke={color} strokeWidth="1.8" strokeLinecap="round"/>
      <Path d="M4 14L9 8L14 12L19 5" stroke={color} strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" opacity="0.55"/>
    </Svg>
  ),
  'Bets': ({color}) => (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
  ...
  'Profile': ({color}) => (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      ...
      <Path d="M5 20C5 17 8.13 15 12 15C15.87 15 19 17 19 20" stroke={color} strokeWidth="1.5" strokeLinecap="round" fill="none"/>
    </Svg>
  ),
};

// Icon style map
const ICON_STYLE_SETS = {
  filled: NAV_ICONS_FILLED,
  outlined: NAV_ICONS_OUTLINED,
  rounded: NAV_ICONS_ROUNDED,
  duotone: NAV_ICONS_DUOTONE,
  sharp: NAV_ICONS_SHARP,
  neon: NAV_ICONS_NEON,
};

const ICON_STYLE_LABELS = {
  filled: 'Filled',
  outlined: 'Outlined',
  rounded: 'Rounded',
  duotone: 'Duotone',
...
let NAV_ICONS = NAV_ICONS_FILLED;

function NavTabItem({icon, name, active, onPress, t, accent, iconStyle='filled'}) {
  const scale = useRef(new Animated.Value(1)).current;
  const glowAnim = useRef(new Animated.Value(0)).current;

  const handlePress = () => {
    Animated.sequence([
      Animated.timing(scale,{toValue:0.78,duration:60,useNativeDriver:true}),
      Animated.spring(scale,{toValue:1,tension:360,friction:10,useNativeDriver:true}),
    ]).start();
    onPress();
  };
  ...
  const iconSet = ICON_STYLE_SETS[iconStyle] || NAV_ICONS_FILLED;
  const IconComp = iconSet[name];
  const iconColor = active ? '#fff' : t.muted;

  const glowRadius = glowAnim.interpolate({inputRange:[0,1],outputRange:[0,18]});
  const glowOpacity = glowAnim.interpolate({inputRange:[0,1],outputRange:[0,0.55]});
  const bgOpacity = glowAnim.interpolate({inputRange:[0,1],outputRange:[0,0.22]});
  ...
  return (
    <TouchableOpacity onPress={handlePress} activeOpacity={1}
      ...
      <Animated.View style={{
        width:44, height:38, borderRadius:18,
        alignItems:'center', justifyContent:'center',
        backgroundColor: withAlpha(accent, 0),
        ...(active ? {
        ...
        {active && <View pointerEvents="none" style={{position:'absolute',top:0,left:'15%',right:'15%',height:1,backgroundColor:'rgba(255,255,255,0.35)',borderRadius:1}}/>}
        <Animated.View style={{transform:[{scale}]}}>
          {IconComp
            ? <IconComp color={iconColor} active={active} accent={accent}/>
...
// ─── ICON STYLE PICKER SHEET ──────────────────────────────────────────────────
function IconStylePicker({visible, onClose, current, onChange, t}) {
  if(!visible) return null;
  const styles2 = Object.keys(ICON_STYLE_SETS);
  const demoNames = ['Home','Insights','Bets','Profile'];
  const demoAccent = t.accent;
  return (
    <Modal transparent animationType="slide" visible={visible} onRequestClose={onClose}>
      ...
      <View style={{
        ...
        ...getDepthStyle('modal','#000'),
      }}>
        <View style={{width:36,height:4,borderRadius:2,backgroundColor:t.muted,alignSelf:'center',marginBottom:18}}/>
        <Text style={{color:t.text,fontSize:17,fontWeight:'700',marginBottom:4,letterSpacing:0.2}}>Nav Icon Style</Text>
        <Text style={{color:t.sub,fontSize:13,marginBottom:20}}>Choose look for navigation icons</Text>
        {styles2.map(styleName=>{
          const iconSet = ICON_STYLE_SETS[styleName];
          const isActive = current===styleName;
          return(
            <TouchableOpacity key={styleName} onPress={()=>{onChange(styleName);onClose();}}
              activeOpacity={0.82}
              style={{
                flexDirection:'row', alignItems:'center',
                paddingVertical:12, paddingHorizontal:14,
                borderRadius:18, marginBottom:10,
                backgroundColor: isActive ? withAlpha(t.accent,0.14) : t.card,
                borderWidth:1.5,
                borderColor: isActive ? withAlpha(t.accent,0.5) : t.inpB,
              }}>
              <View style={{flex:1}}>
                <View style={{flexDirection:'row',alignItems:'center',gap:6,marginBottom:2}}>
                  <Text style={{color:t.text,fontSize:15,fontWeight:'700'}}>{ICON_STYLE_LABELS[styleName]}</Text>
                  {isActive && <View style={{backgroundColor:t.accent,borderRadius:8,paddingHorizontal:8,paddingVertical:2}}>
              ...
              <View style={{flexDirection:'row',gap:14,alignItems:'center'}}>
                {demoNames.map(n=>{
                  const IC = iconSet[n];
                  return IC
                    ? <IC key={n} color={isActive ? demoAccent : t.muted} active={false} accent={demoAccent}/>
                    : null;
                })}
              </View>
            </TouchableOpacity>
...
export default function App() {
  ...
  if(!loaded) return (
    <View style={{flex:1,backgroundColor:'#000',justifyContent:'center',alignItems:'center'}}>
      <GradientBackground t={THEMES.amoled}/>
      <View style={{alignItems:'center'}}>
        <View style={{width:88,height:88,borderRadius:28,backgroundColor:'rgba(10,132,255,0.15)',borderWidth:1.5,borderColor:'rgba(10,132,255,0.35)',justifyContent:'center',alignItems:'center',marginBottom:20,shadowColor:'#0a84ff',shadowOpacity:0.4,shadowRadius:24,shadowOffset:{width:0,height:8}}}>
          <Text style={{fontSize:46}}>🏆</Text>
        </View>
        <Text style={{color:'#fff',fontSize:22,fontWeight:'900',letterSpacing:-0.5}}>BetTracker Pro</Text>
        <Text style={{color:'rgba(255,255,255,0.4)',fontSize:12,marginTop:8,fontWeight:'600'}}>Loading your stats...</Text>
        <View style={{marginTop:24,flexDirection:'row',gap:6}}>
          {[0,1,2].map(i=>{
            const dot = useRef(new Animated.Value(0.3)).current;
            useEffect(()=>{
              const loop = Animated.loop(Animated.sequence([
                Animated.timing(dot,{toValue:1,duration:500,delay:i*160,useNativeDriver:false}),
                Animated.timing(dot,{toValue:0.3,duration:500,useNativeDriver:false}),
              ]));
              loop.start(); return ()=>loop.stop();
            },[]);
            return <Animated.View key={i} style={{width:6,height:6,borderRadius:3,backgroundColor:'#0a84ff',opacity:dot}}/>;
          })}
        </View>
      </View>
    </View>
  ...
  return (
    <SafeAreaProvider>
      ...
      <View style={{flex:1,backgroundColor:t.bg}}>
        ...
        <SafeAreaView style={{flex:1}} edges={['top']}>
          ...
          >
            ...
            {tab===0&&<Dashboard bets={bets} t={t}
              ...
              onSettlePending={()=>changeTab(3)}
              onGoToBets={()=>changeTab(3)}
              onGoToStats={()=>changeTab(1)}
            />}
        ...
        {/* ── FAB action bottom sheet (Change #11) ─────────────────────── */}
        {showFab&&(
          <Animated.View style={{
            position:'absolute',bottom:106,left:14,right:14,
            zIndex:300,
            borderRadius:28,
            borderWidth:1,
            borderColor:'rgba(255,255,255,0.10)',
            ...
            {/* Top shimmer */}
            <View pointerEvents="none" style={{position:'absolute',top:0,left:20,right:20,height:1,backgroundColor:'rgba(255,255,255,0.14)',zIndex:3}}/>
            {/* Header */}
            <View style={{paddingHorizontal:20,paddingTop:16,paddingBottom:10,borderBottomWidth:1,borderBottomColor:'rgba(255,255,255,0.06)'}}>
              <View style={{flexDirection:'row',alignItems:'center',justifyContent:'space-between'}}>
                <Text style={{fontSize:13,fontWeight:'900',color:'rgba(255,255,255,0.9)',letterSpacing:0.2}}>Quick Actions</Text>
                <View style={{backgroundColor:'rgba(255,255,255,0.07)',borderRadius:8,paddingHorizontal:10,paddingVertical:4}}>
                  <Text style={{fontSize:10,color:'rgba(255,255,255,0.4)',fontWeight:'700'}}>TAP TO SELECT</Text>
                </View>
              </View>
            </View>
            {[
              {icon:'➕',label:'Add Bet',    sub:'Full form with all fields',     color:'#30d158', bg:'rgba(48,209,88,0.12)',   border:'rgba(48,209,88,0.28)',   fn:()=>{setEditBet(null);setShowModal(true);setShowFab(false);}},
              {icon:'⚡',label:'Quick Add',  sub:'Fast 3-field entry',            color:'#0a84ff', bg:'rgba(10,132,255,0.12)',  border:'rgba(10,132,255,0.28)',  fn:()=>{setShowQuick(true);setShowFab(false);}},
              {icon:'🎫',label:'Bet Slip',   sub:'Track a multi-leg accumulator', color:'#ffd60a', bg:'rgba(255,214,10,0.12)',  border:'rgba(255,214,10,0.28)',  fn:()=>{setShowSlip(true);setShowFab(false);}},
              {icon:'📋',label:'Templates',  sub:'Reuse a saved bet template',    color:'#bf5af2', bg:'rgba(191,90,242,0.12)', border:'rgba(191,90,242,0.28)', fn:()=>{setShowTemplates(true);setShowFab(false);}},
            ].map((item,i,arr)=>(
              <TouchableOpacity key={i} onPress={item.fn} activeOpacity={0.78}
                style={{
                ...
                <View style={{
                  ...
                  justifyContent:'center', alignItems:'center',
                  shadowColor:item.color, shadowOpacity:0.35, shadowRadius:10, shadowOffset:{width:0,height:3},
                }}>
                ...
                <View style={{flex:1}}>
                  ...
                  <Text style={{fontSize:11, color:'rgba(255,255,255,0.35)', marginTop:2, fontWeight:'500'}}>{item.sub}</Text>
                </View>
                {/* Color arrow */}
                <View style={{width:28,height:28,borderRadius:9,backgroundColor:item.bg,borderWidth:1,borderColor:item.border,justifyContent:'center',alignItems:'center'}}>
                  <Text style={{fontSize:14, color:item.color, fontWeight:'900'}}>›</Text>
                </View>
              </TouchableOpacity>
            ))}
            <View style={{paddingHorizontal:18,paddingBottom:16,paddingTop:10}}>
              <Text style={{fontSize:10,color:'rgba(255,255,255,0.2)',textAlign:'center',fontWeight:'600'}}>Tap outside to dismiss</Text>
            </View>
          </Animated.View>
        )}
        ...
        <SafeAreaView edges={['bottom']} style={{position:'absolute',bottom:0,left:0,right:0,zIndex:100}}>
          <View style={{paddingHorizontal:12,paddingBottom:10,paddingTop:6}}>
            <View style={getDepthStyle('nav', isLightTheme(t) ? 'rgba(90,110,255,0.34)' : '#000')}>
              ...
              {/* Home */}
              <NavTabItem icon="🏠" name="Home" active={tab===0} onPress={()=>changeTab(0)} t={t} accent={t.accent} iconStyle={iconStyle}/>

              {/* Insights */}
              <NavTabItem icon="📊" name="Insights" active={tab===1} onPress={()=>changeTab(1)} t={t} accent={t.accent} iconStyle={iconStyle}/>

              {/* Center FAB — gradient Add button */}
              <View style={{width:56,alignItems:'center',justifyContent:'center'}}>
                ...
                >
                  ...
                  <View pointerEvents="none" style={{position:'absolute',top:0,left:0,right:0,height:26,borderRadius:26,backgroundColor:'rgba(255,255,255,0.22)'}}/>
                  <GlassLayer t={t} surface='button' borderRadius={26} blurAmount={18} tone={showFab ? '#ff453a' : t.accent}/>
                  <Text style={{color:'#fff',fontSize:28,fontWeight:'100',lineHeight:32,includeFontPadding:false,zIndex:2}}>
                    {showFab ? '✕' : '＋'}
                  </Text>
                </TouchableOpacity>
              </View>
              ...
              {/* Bets */}
              <NavTabItem icon="📋" name="Bets" active={tab===3} onPress={()=>changeTab(3)} t={t} accent={t.accent} iconStyle={iconStyle}/>

              {/* Profile */}
              <NavTabItem icon="👤" name="Profile" active={tab===4} onPress={()=>changeTab(4)} t={t} accent={t.accent} iconStyle={iconStyle}/>
                </View>
              </View>
            </View>
          </View>
