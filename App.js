...
export default function App() {
  ...
  const loadHTML = async () => {
    try {
      ...
      // Always read fresh from asset (don't cache to avoid stale file bug)
      let content = await FileSystem.readAsStringAsync(asset.localUri);

      // Remove Cloudflare beacon (not needed in app)
      content = content.replace(/<script defer src="https:\/\/static\.cloudflareinsights\.com\/.*?<\/script>/s, '');

      // Inject localStorage polyfill at the top of <head> so data persists
      const localStoragePolyfill = `
<script>
(function(){
  var _store = {};
  try { _store = JSON.parse(window._lsData || '{}'); } catch(e){}
  var ls = {
    getItem: function(k){ return _store[k] !== undefined ? _store[k] : null; },
    setItem: function(k,v){ _store[k]=String(v); try{window.ReactNativeWebView&&window.ReactNativeWebView.postMessage(JSON.stringify({type:'ls_set',key:k,value:String(v)}));}catch(e){} },
    removeItem: function(k){ delete _store[k]; },
    clear: function(){ _store={}; },
    key: function(i){ return Object.keys(_store)[i]||null; },
    get length(){ return Object.keys(_store).length; }
  };
  Object.defineProperty(window,'localStorage',{get:function(){return ls;}});
  Object.defineProperty(window,'sessionStorage',{get:function(){return ls;}});
})();
</script>`;

      // Inject polyfill right after <head>
      content = content.replace('<head>', '<head>' + localStoragePolyfill);
  ...
  const handleMessage = (event) => {
    try {
      ...
      if (data.type === 'ls_set') {
        // Persist to AsyncStorage or FileSystem if needed
        // For now data lives in memory per session
      }
    } catch(e) {}
  };
      ...
      <SafeAreaView style={styles.container}>
        {error ? (
          <View style={[styles.container, {alignItems:'center', justifyContent:'center'}]}>
            <Text style={{color:'#ff453a', fontSize:13, textAlign:'center', padding:20}}>
              Load error: {error}
            </Text>
          </View>
        ) : htmlContent ? (
          <WebView
            ...
            onError={(e) => {
              console.error('WebView error:', JSON.stringify(e.nativeEvent));
              setError(e.nativeEvent.description);
            }}
            renderLoading={() => (
              <View style={[styles.container, {alignItems:'center', justifyContent:'center', position:'absolute', top:0, left:0, right:0, bottom:0}]}>
                <Text style={{color:'#0a84ff', fontSize:16}}>🏆 Loading...</Text>
              </View>
            )}
          ...
          <View style={[styles.container, {alignItems:'center', justifyContent:'center'}]}>
            <Text style={{color:'#636366', fontSize:13}}>Preparing app...</Text>
          </View>
        )}
      </SafeAreaView>
