...
function Dashboard({bets, t, onAddBet, onQuickAdd, onSettlePending, onGoToBets, onGoToStats}) {
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false}>
      ...
      {/* ── Mini Stats Row ── */}
      <View style={{flexDirection:'row',marginBottom:16,marginHorizontal:-3}}>
        ...
        <StatCard label="Today"       value={fcs(todayPL)}                      sub={`${todayBets.length} bets`} color={todayPL>=0?'#30d158':'#ff453a'} icon="📅" t={t}/>
      </View>
      ...
      {runData.length>=2&&<Card t={t} style={{marginBottom:12}}>
        <SectionHead title="📈 Performance" sub={`Max DD: ${fc(maxDD)}`} t={t}/>
