...
function GlassLayer({t, borderRadius=22, blurAmount=22}) {
  const overlayStyle = {
    ...StyleSheet.absoluteFillObject,
    borderRadius,
    borderWidth:1,
    borderColor:t.cardB,
    backgroundColor: BlurView ? getGlassOverlay(t) : t.card,
  };

  if (!BlurView) return <View pointerEvents="none" style={overlayStyle}/>;
  ...
  return (
    <>
      <BlurView
        ...
        blurAmount={Platform.OS === 'android' ? Math.max(blurAmount, 26) : blurAmount}
  ...
  return (
    <Animated.View style={[
      ...
      <View style={{borderRadius:22,overflow:'hidden'}}>
        <GlassLayer t={t} borderRadius={22} blurAmount={24}/>
        <TouchableOpacity
  ...
  return (
    <View style={{
      ...
      <View style={{borderRadius:20,overflow:'hidden'}}>
        <GlassLayer t={t} borderRadius={20} blurAmount={20}/>
...
function HeroCard({tPL, roi, wr, won, lost, pending, t}) {
  ...
  // Breathing glow pulse (from BettingTrackerDashboard HeroPnL)
  const glowOpacity = useRef(new Animated.Value(0.35)).current;
  useEffect(() => {
    const loop = Animated.loop(Animated.sequence([
      Animated.timing(glowOpacity, {toValue:0.65, duration:2000, useNativeDriver:false}),
      Animated.timing(glowOpacity, {toValue:0.35, duration:2000, useNativeDriver:false}),
    ]));
    loop.start();
    return () => loop.stop();
  }, []);

  // Subtle scale breathe on profit
  const pulseAnim = useRef(new Animated.Value(1)).current;
  useEffect(() => {
    if (!isProfit) { pulseAnim.setValue(1); return; }
    const loop = Animated.loop(Animated.sequence([
      Animated.timing(pulseAnim, {toValue:1.016, duration:1800, useNativeDriver:true}),
  ...
  return (
    <View style={{position:'relative', marginBottom:16}}>
      {/* Premium breathing glow orb behind card (BettingTrackerDashboard style) */}
      <Animated.View style={{
        position:'absolute', top:-28, left:'8%',
        width:'84%', height:170, borderRadius:90,
        backgroundColor: isProfit ? 'rgba(0,217,139,0.16)' : 'rgba(255,77,106,0.16)',
        opacity: glowOpacity,
      }}/>
      ...
      <Animated.View style={{
        ...
        shadowColor: plC, shadowOpacity:0.50, shadowRadius:40, shadowOffset:{width:0, height:12},
      }}>
        {/* Main card body */}
        <View style={{
          borderRadius:28, padding:24,
          backgroundColor: isProfit ? 'rgba(18,42,26,0.97)' : 'rgba(44,16,20,0.97)',
          borderWidth:1.5, borderColor: plC + '50',
          overflow:'hidden',
        }}>
          {/* Inner ambient corners */}
          <View style={{position:'absolute',top:-40,left:-30,width:180,height:180,borderRadius:90,backgroundColor:plC,opacity:0.06}}/>
          <View style={{position:'absolute',bottom:-50,right:-20,width:140,height:140,borderRadius:70,backgroundColor:plC,opacity:0.04}}/>
          {/* Accent gradient bar at top */}
          <View style={{position:'absolute',top:0,left:'12%',right:'12%',height:1.5,backgroundColor:plC,opacity:0.45,borderRadius:1}}/>

          {/* Header row */}
          <View style={{flexDirection:'row', justifyContent:'space-between', alignItems:'center', marginBottom:8}}>
            <Text style={{fontSize:10, color:'rgba(255,255,255,0.32)', fontWeight:'800', letterSpacing:2, textTransform:'uppercase'}}>Total P&L</Text>
            <View style={{flexDirection:'row',gap:7}}>
              <View style={{backgroundColor: plC+'28', borderRadius:10, paddingHorizontal:10, paddingVertical:4, borderWidth:1, borderColor:plC+'42'}}>
                <Text style={{fontSize:10, color:plC, fontWeight:'800', letterSpacing:0.3}}>ROI {pct(roi)}</Text>
              </View>
              <View style={{backgroundColor:'rgba(255,255,255,0.07)',borderRadius:10,paddingHorizontal:10,paddingVertical:4,borderWidth:1,borderColor:'rgba(255,255,255,0.10)'}}>
                <Text style={{fontSize:10,color:'rgba(255,255,255,0.48)',fontWeight:'700'}}>WR {pct(wr,0)}</Text>
              </View>
            </View>
          </View>

          {/* Big P&L number — large count-up */}
          <Text style={{fontSize:54, fontWeight:'900', color:plC, letterSpacing:-2.5, marginBottom:18, includeFontPadding:false, lineHeight:60}}>
            {sign}{absStr}
          </Text>
          ...
          <View style={{flexDirection:'row'}}>
            {[
              ...
              {label:'Win Rate', value:pct(wr,0),       color:'#4F6EF7'},
            ].map((item,i) => (
              <View key={i} style={{flex:1, alignItems:'center', gap:3}}>
                <Text style={{fontSize:20, fontWeight:'900', color:item.color, letterSpacing:-0.5}}>{item.value}</Text>
                <Text style={{fontSize:8, color:'rgba(255,255,255,0.28)', fontWeight:'700', letterSpacing:0.9, textTransform:'uppercase'}}>{item.label}</Text>
              </View>
            ))}
          </View>
        </View>
...
function Dashboard({bets, t, onAddBet, onQuickAdd, onSettlePending, onGoToBets, onGoToStats}) {
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false}>
      ...
      {/* ── Premium ambient background glow orbs (BettingTrackerDashboard) ── */}
        ...
        <SafeAreaView edges={['bottom']} style={{position:'absolute',bottom:0,left:0,right:0,zIndex:100}}>
          <View style={{paddingHorizontal:12,paddingBottom:10,paddingTop:6}}>
            <View style={{borderRadius:44,overflow:'hidden'}}>
              <GlassLayer t={t} borderRadius={44} blurAmount={30}/>
              <View style={{
